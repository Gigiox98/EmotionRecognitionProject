...
app = Flask(__name__)
app.secret_key="deuiSD6952fiuShfesi@fhes#+"
cron = Scheduler()
cron.start()
lock=threading.Lock()
...
def get_log(file_id):
	for f in os.listdir(os.getcwd()+"\\static"):
		if f.endswith(str(file_id)+".txt"):
...
def job_function():
	for f in os.listdir(os.getcwd()+"/static"):
		if not(f.endswith("affective.jpg") or f.endswith(".txt")): 
			if platform.system() == 'Windows':
				stat = os.path.getctime(os.path.join(os.getcwd()+"/static",f))
			else:
				stat = os.stat.st_mtime(os.path.join(os.getcwd()+"/static",f))

			if time.time()-stat>10800:
				os.remove(os.path.join(os.getcwd()+"/static",f))
				print(os.path.join(os.getcwd()+"/static",f)+" removed")

	for f in os.listdir(os.getcwd()):
		if not(f.endswith("static") or f.endswith("app.py") or f.endswith("templates") or f.endswith("ft")): 
			if platform.system() == 'Windows':
				stat = os.path.getctime(os.path.join(os.getcwd(),f))
			else:
				stat = os.stat.st_mtime(os.path.join(os.getcwd(),f))

			if time.time()-stat>3600:
				os.remove(os.path.join(os.getcwd(),f))
				print(os.path.join(os.getcwd(),f)+" removed")
